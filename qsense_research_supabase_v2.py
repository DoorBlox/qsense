...
SUPABASE_SYNC_INTERVAL = 5   # seconds
...
# Set QSENSE_TEST_MODE=true in .env to test outside meal hours.
# In test mode, Q-SENSE performs detection/tracking and updates live_status,
# but it does NOT write research CSV files, queue_events, or queue_snapshots.
TEST_MODE = os.getenv("QSENSE_TEST_MODE", "false").strip().lower() in {"1", "true", "yes", "on"}
...
def supabase_execute(action, description):
    """Run a Supabase action without crashing the local tracker on network errors."""
    try:
        return action.execute()
    except Exception as e:
        print(f"SUPABASE WARNING ({description}): {e}")
        return None


def mark_offline():
    """Best-effort shutdown update. Website should still use updated_at as the real heartbeat."""
    try:
        supabase.table("live_status").update({
            "updated_at": datetime.now().isoformat(timespec="seconds"),
            "queue_status": "offline",
            "camera_online": False,
        }).eq("id", 1).execute()
        print("Q-SENSE marked offline in Supabase.")
    except Exception as e:
        print(f"SUPABASE WARNING (mark_offline): {e}")


atexit.register(mark_offline)


def push_event(session_id, timestamp, track_id, event, wait_seconds, queue_count):
    if TEST_MODE:
        return
    ...
    supabase_execute(
        supabase.table("queue_events").insert(payload),
        f"queue_events/{event}",
    )
...
last_snapshot = 0
last_supabase_sync = 0
last_pushed_queue_count = None
...
if TEST_MODE:
    ...
    print("Tracking is enabled outside meal hours.")
    print("Research CSV/events/snapshots are NOT being recorded.")
print("")
...
while True:
    ...
    if meal == "outside_meal":
        ...
        if now_timestamp - last_supabase_sync >= SUPABASE_SYNC_INTERVAL:
            supabase_execute(
                supabase.table("live_status").update({
                    "updated_at": now.isoformat(timespec="seconds"),
                    "meal_period": "outside_meal",
                    "queue_count": 0,
                    "queue_status": "outside_meal",
                    "trend": "stable",
                    "served": 0,
                    "avg_wait_seconds": 0,
                ...
                "live_status/outside_meal",
            )
            last_supabase_sync = now_timestamp
            last_pushed_queue_count = 0
    ...
    if (
        ...
        if not TEST_MODE:
            supabase_execute(
                supabase.table("queue_snapshots").insert({
    ...
    if now_timestamp - last_supabase_sync >= SUPABASE_SYNC_INTERVAL:
        if last_pushed_queue_count is None:
            trend = "stable"
        elif queue_count > last_pushed_queue_count:
            trend = "rising"
        elif queue_count < last_pushed_queue_count:
            trend = "falling"
        else:
        ...
        supabase_execute(
            supabase.table("live_status").update({
                ...
                "meal_period": meal,
                "queue_count": int(queue_count),
                "queue_status": get_queue_status(queue_count),
                "trend": trend,
                "served": int(total_served),
                "avg_wait_seconds": round(float(avg_wait), 2),
                "throughput_per_min": float(throughput),
                "camera_online": True,
            }).eq("id", 1),
            "live_status/update",
        ...
        last_supabase_sync = now_timestamp
        last_pushed_queue_count = queue_count
